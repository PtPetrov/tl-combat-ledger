...
export async function parseLogFileSummary(
  ...
  // drop "CombatLogVersion,4" if present
  const first = allLines[0];
  const linesWithoutVersion = first.startsWith("CombatLogVersion")
    ? allLines.slice(1)
    : allLines;

  const header =
    "Timestamp,LogType,SkillName,SkillId,Damage,HitCritical,HitDouble,HitType,CasterName,TargetName";
  const csvText = [header, ...linesWithoutVersion].join("\n");
  ...
  for (const row of rows) {
    ...
    if (logType !== "DamageDone") continue;
  ...
  const toSkillBreakdown = (s: SkillAccumulator): SkillBreakdown => {
    ...
    const critRate = s.totalHits > 0 ? (s.critHits / s.totalHits) * 100 : 0;
    const heavyRate = s.totalHits > 0 ? (s.heavyHits / s.totalHits) * 100 : 0;
    const normalAvg =
      s.normalHits > 0
  ...
  const targets: TargetBreakdown[] = Array.from(targetsMap.values())
    .map((t) => ({
      ...
      avgHit: t.totalHits > 0 ? Number((t.sumHit / t.totalHits).toFixed(1)) : 0,
      critHits: t.critHits,
      critRate:
        t.totalHits > 0
  ...
  if (firstTimestamp != null) {
    perTargetSessionsState.forEach((state, targetName) => {
      const summaries: TargetSessionSummary[] = state.sessions
        ...
        .map((s) => {
          const skillsArray: SkillBreakdown[] = Array.from(s.skills.values())
            .map(toSkillBreakdown)
            .sort((a, b) => b.totalDamage - a.totalDamage);
          ...
          const critRate =
            s.totalEvents > 0
              ? Number(((s.totalCritHits / s.totalEvents) * 100).toFixed(1))
              : 0;

          return {
            sessionId: s.id,
            startElapsed: (s.startMs - firstTimestamp) / 1000,
            endElapsed: (s.endMs - firstTimestamp) / 1000,
  ...
  return {
    ...
    startTime:
      firstTimestamp != null ? new Date(firstTimestamp).toISOString() : null,
    endTime:
    ...
    hps: null,
    critRate: globalCritRate != null ? Number(globalCritRate.toFixed(1)) : null,
    skills,
    targets,
    perTargetSkills,
    perTargetSessions,
